"""Genera `caso-mercato.html`: l'indice usato per rispondere a una domanda di mercato.
    ...
    python caso_mercato.py

Legge `payload_lista.json` — tutti i 354 giocatori qualificati, non i primi 100 —
e scrive la
...
PAYLOAD = QUI / "payload_lista.json"   # tutti i 354, non i primi 100
PAYLOAD_PREC = QUI / "payload_lista_2024-25.json"  # per verificare se i valori si muovono
OUT = QUI / "caso-mercato.html"
...
def main():
    ...
    nota_gen = el("p",
        "Pagina generata da <code>caso_mercato.py</code> a partire da <code>payload_lista.json</code>: "
        "nessun numero &egrave; scritto a mano.",
        "Page generated by <code>caso_mercato.py</code> from <code>payload_lista.json</code>: no "
...
<title>Con {BUDGET:.0f} milioni, questi tre &mdash; Serie A Scout Index</title>
...
<main class="cm-w">
  <header style="padding:56px 0 8px">
    <div class="cm-occhiello" data-it="Serie A Scout Index &middot; Caso di mercato" data-en="Serie A Scout Index &middot; Market case">Serie A Scout Index &middot; Caso di mercato</div>
